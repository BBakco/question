...
def login():
    email_receive = request.form['email_give']
    pw_receive = request.form['pw_give']
    user_data = list(db.question.find({}, {'_id': False}))
    ...
    if "@" and "." not in email_receive:
        ...
        for user in user_data:
            if email_receive == user['email'] and pw_receive == user['pw']:
                return jsonify({'msg': '환영합니다.'})
            elif email_receive != user['email'] or pw_receive != user['pw']:
                return jsonify({'msg': '입력하신 정보를 확인해주세요.'})
            # elif email_receive == user['email'] and pw_receive != user['pw']:
            #     return jsonify({'msg': '입력하신 정보를 확인해주세요.'})
            # elif email_receive != user['email'] and pw_receive == user['pw']:
            #     return jsonify({'msg': '입력하신 정보를 확인해주세요.'})
